...
import { createEdgingLog } from '../actions'
...
export default function EdgingPage() {
  ...
  const [todayEntries, setTodayEntries] = useState<EdgingEntry[]>([])
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: entries } = await supabase
        .from('edging_logs')
        .select('id, event_time, notes, rink:rinks(name), operator:profiles(full_name)')
        .eq('facility_id', profile.facility_id)
        .gte('event_time', todayStart.toISOString())
        .order('event_time', { ascending: false })

      if (entries) setTodayEntries(entries as unknown as EdgingEntry[])
    } catch {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!rinkId) {
      toast({ title: 'Validation Error', description: 'Please select a rink.', variant: 'destructive' })
      return
    }

    setSubmitting(true)
    try {
      const result = await createEdgingLog({
        rink_id: rinkId,
      ...
      if (result.success) {
        toast({ title: 'Edging Logged', description: 'Entry has been recorded successfully.', variant: 'success' })
        // Clear form
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                id="notes"
                placeholder="Optional notes about this edging..."
                value={notes}
